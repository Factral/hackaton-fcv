...
export default function MyChat () {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    const mensajeEnviar = { id: new Date().getTime(), mensaje: nuevoMensaje, fecha: '2021-10-10', hora: '10:00' }
    ...
    <div className='p-3'>
      <div className='w-full max-w-4xl bg-gradient-to-tr relative to-main-white-100 rounded-xl from-main-white  h-[calc(100vh-158px)] m-auto p-3'>
        <div className='flex flex-col justify-end h-full'>
          <div className='overflow-y-auto flex flex-col clean-scroll show-scroll p-3 mb-2' ref={ref}>
            {mensajes.map((mensaje) => (
              <div key={mensaje.id} className='flex flex-col'>
                <div className='flex justify-between'>
                  <p className='text-sm text-main-blue-100'>{mensaje.fecha}</p>
                  <p className='text-sm text-main-blue-100'>{mensaje.hora}</p>
                </div>
                <p className='text-sm text-main-blue-100'>{mensaje.mensaje}</p>
              </div>
            ))}
          </div>
          <form onSubmit={handleSubmit} className='flex relative'>
            <textarea name='' id='' onChange={(e) => setNuevoMensaje(e.target.value)} value={nuevoMensaje} rows='1' className='w-full outline-none rounded-md resize-none py-3 pl-6 pr-[52px] min-h-[10px] text-lg font-medium clean-scroll' />
